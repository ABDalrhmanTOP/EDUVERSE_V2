// Improved Extract Course Info Node - Better Caption Handling
...
if (!courseData.youtube_url) {
  throw new Error('Missing required course information: youtube_url is required');
}
...
// Check if we have real captions or just description
const hasRealCaptions = courseData.video_captions && 
  (courseData.video_captions.includes('0:00') || 
   courseData.video_captions.includes('00:') ||
   courseData.video_captions.includes('1:') ||
   courseData.video_captions.includes('2:') ||
   courseData.video_captions.includes('3:') ||
   courseData.video_captions.includes('4:') ||
   courseData.video_captions.includes('5:') ||
   courseData.video_captions.includes('6:') ||
   courseData.video_captions.includes('7:') ||
   courseData.video_captions.includes('8:') ||
   courseData.video_captions.includes('9:')) &&
  courseData.video_captions.length > 1000;

const captionsSource = hasRealCaptions ? 'REAL_CAPTIONS' : 'VIDEO_DESCRIPTION';
...
console.log('Caption analysis:', {
  ...
  has_timestamps: courseData.video_captions ? 
    (courseData.video_captions.includes('0:00') || courseData.video_captions.includes('00:')) : false,
  captions_source: captionsSource,
  captions_preview: courseData.video_captions ? courseData.video_captions.substring(0, 200) + '...' : 'None',
  is_real_captions: hasRealCaptions
});
...
const preparedCourseData = {
  ...
  description: courseData.video_description || 'Educational course extracted from YouTube video content',
...
const result = {
  ...
  video_duration: courseData.video_duration,
  captions_source: captionsSource, // CRITICAL: Tell AI if we have real captions
  ai_context: {
    video_title: courseData.video_title,
    video_description: courseData.video_description,
    video_channel: courseData.video_channel,
    ...
    captions_source: captionsSource,
    has_real_captions: hasRealCaptions
  }
};
...
console.log('Returning result with captions and duration:', {
  ...
  captions_source: result.captions_source,
  has_real_captions: result.ai_context.has_real_captions,
