  ...
  >
    ...
    <div className="absolute inset-0 z-10 bg-black bg-opacity-60 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-6">
      ...
      <p className="text-white text-sm mb-2">
        Average Rating:{" "}
        {playlist.average_rating ? playlist.average_rating : "N/A"}
      </p>
      <button
        onClick={(e) => {
          e.stopPropagation();
...
const UserProgressTable = () => {
  // ... your existing admin progress code ...
};
...
const HomeVideo = () => {
  ...
  // EXAMPLE: Always start user at level 1 test
  const handleCourseClick = (playlist) => {
    if (!isAuthenticated) {
      navigate("/login");
    } else {
      // If user hasn't done the test, we can start at level 1
      // If your logic depends on something else, adjust accordingly
      navigate("/form_test/1");
    }
  };
